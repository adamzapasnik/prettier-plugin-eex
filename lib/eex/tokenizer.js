...
function tokenize(text) {
  ...
  const rawElementsInText = ['pre', 'code', 'samp', 'kbd', 'var', 'ruby', 'noscript', 'canvas', 'style', 'title'].filter((tag) =>
    uniqueClosedTags.has(`</${tag}>`)
  );
  ...
  text.replace(regexp, (match, offset) => {
    ...
    const afterWhitespace = lastTokenType === 'multiline_expr' || (lastTokenType === 'text' && /\s/.test(html[html.length - 1]));
    ...
    tokens.push({
      ...
      inComment: inComment,
      inElementWithoutNeedToEncode: !inElement && !inComment && rawElementsInText.find((tag) => isWithinElement(html, tag)),
    });
  });
